...
def custom_error_handling(bot, logger):
    ...
    async def on_command_error(ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            msg = _('Meowth! Team Management is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, WantSetCheckFail):
            msg = _('Meowth! Pokemon Notifications are not enabled on this server. **{prefix}{cmd_name}** is unable to '
                    'be used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, WildSetCheckFail):
            msg = _('Meowth! Wild Reporting is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, ReportCheckFail):
            msg = _('Meowth! Reporting is not enabled for this channel. **{prefix}{cmd_name}** is unable to be used.')\
                  .format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, RaidSetCheckFail):
            msg = _('Meowth! Raid Management is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, EXRaidSetCheckFail):
            msg = _('Meowth! EX Raid Management is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, ResearchSetCheckFail):
            msg = _('Meowth! Research Reporting is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, MeetupSetCheckFail):
            msg = _('Meowth! Meetup Reporting is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, ArchiveSetCheckFail):
            msg = _('Meowth! Channel Archiving is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, InviteSetCheckFail):
            msg = _('Meowth! EX Raid Invite is not enabled on this server. **{prefix}{cmd_name}** is unable to be '
                    'used.').format(cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in the following channel').format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in a Raid channel. Use **{prefix}list** in any ').format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            if len(city_channels) > 10:
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in an Egg channel. Use **{prefix}list** in any ').format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            if len(city_channels) > 10:
                msg += _('Region report channel to see active raids.')
            ...
            msg = _("Meowth! **{prefix}{cmd_name}** can't be used in a Raid channel.").format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in an Active Raid channel. Use **{prefix}list** in '
                    'any ').format(cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            try:
            ...
            if egg_check == "egg" and not meetup:
                msg += _('\nThis is an egg channel. The channel needs to be activated with **{prefix}raid <pokemon>** '
                         'before I accept commands!').format(prefix=prefix)
            error = await ctx.channel.send(msg)
            await asyncio.sleep(10)
            await delete_error(ctx.message, error)
        elif isinstance(error, ActiveChannelCheckFail):
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in an Active channel. Use **{prefix}list** in '
                    'any ').format(cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            try:
                egg_check = bot.guild_dict[guild.id]['raidchannel_dict'][ctx.channel.id].get('type', None)
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in either a Raid channel or ').format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            if len(city_channels) > 10:
                msg += _('a Region report channel.')
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in either a Raid Egg channel or ').format(
                cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['raid']['report_channels']
            if len(city_channels) > 10:
                msg += _('a Region report channel.')
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in either a EX Raid channel or one of the following '
                    'region channels:').format(cmd_name=ctx.invoked_with, prefix=prefix)
            ...
            guild = ctx.guild
            msg = _('Meowth! Please use **{prefix}{cmd_name}** in a EX Raid channel. Use **{prefix}list** in any of the'
                    ' following region channels to see active raids:').format(cmd_name=ctx.invoked_with, prefix=prefix)
            city_channels = bot.guild_dict[guild.id]['configure_dict']['exraid']['report_channels']
            if len(city_channels) > 10:
                msg += _('a Region report channel.')
            else:
